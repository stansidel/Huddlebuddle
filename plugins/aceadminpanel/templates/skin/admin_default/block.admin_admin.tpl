...
<div class="block white">
    ...
    <div class="cl"><div class="cr">
            <h2>{$oLang->adm_users_action} &darr;</h2>
            <div>

                <h3><span id="admin-block-a1"><a href="#" onclick="AdminAction(1); return false;">{$oLang->adm_seek}</a></span><span id="admin-block-t1" style="display:none;">{$oLang->adm_seek}</span></h3>
                <form method="post" action="{router page='admin'}users/" id="admin_form_seek">
					...
					<span class="form_note">{$oLang->adm_user_mail_seek_notice}</span>
                        </p -->
                        <p>
                ...
                <div {if $sMode!='admins'}style="display:none;"{/if}>
                    <h3><span id="admin-block-a2"><a href="#" onclick="AdminAction(2); return false;">{$oLang->adm_include}</a></span><span id="admin-block-t2" style="display:none;">{$oLang->adm_include}</span></h3>
                    <form method="post" action="">
                        <input type="hidden" name="security_ls_key" value="{$LIVESTREET_SECURITY_KEY}" />
                        <div id="admin-block-d2" style="margin-left:20px;display:none;">
                            <p>
                                <label for="user_login_admin">{$oLang->adm_user_login}</label><input type="text" name="user_login_admin" id="user_login_admin" maxlength="30" style="width:250px;" /><br />
                            </p>
                            <p>
                                <input type="hidden" name="adm_user_ref" value="{$sPageRef}" />
                    ...
                    <form method="post" action="">
                        ...
                        <div id="admin-block-d3" style="margin-left:20px;display:none;">
                            ...
                            <p>
                                ...
                                <label for="send_common_message_yes">{$oLang->adm_send_common_message}</label><br />
                                ...
                                <label for="send_common_message_no">{$oLang->adm_send_separate_messages}</label><br />
                                <span id="send_common_notice" class="form_note" style="display:none;">{$oLang->adm_send_common_notice}</span>
                                <span id="send_separate_notice" class="form_note">{$oLang->adm_send_separate_notice}</span>
                            </p>
                            <p>
                            ...
                            <p>
                                ...
                                <label for="send_copy_self">{$oLang->adm_send_copy_self}</label>
                            </p>
                            <input type="hidden" name="adm_user_ref" value="{$sPageRef}" />
