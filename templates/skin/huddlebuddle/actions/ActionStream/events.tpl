{if count($aStreamEvents)}
		{foreach from=$aStreamEvents item=oStreamEvent}		
                        ...
                        {assign var=oUser value=$oTarget->GetUser()}
                        {if $oStreamEvent->getEventType() == 'add_topic' || $oStreamEvent->getEventType() == 'vote_topic'}
                            ...
                            {if $showSingleAdmin && !$oUserCurrent->IsAdministrator()}
                                {continue}
                            {/if}
                        {elseif $oStreamEvent->getEventType() == 'add_comment' || $oStreamEvent->getEventType() == 'vote_comment'}
                            ...
                            {if $oTarget->getTargetType() == "topic"}
                                {$showSingleAdmin = $showSingleAdmin && ($oTarget->getTarget()->GetUserId() != $oUser->getId() || $oConfig->GetValue('admin_actions.single_topics'))}
                            {/if}
                            {if $showSingleAdmin && !$oUserCurrent->IsAdministrator()}
                                {continue}
                            {/if}
